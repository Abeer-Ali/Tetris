...
public class Square {
	...
	public void draw(Graphics g, double scaleWidth, double scaleHeight) {
		...
		int actualX = (int)(Grid.LEFT * scaleWidth + col * WIDTH * scaleWidth);
		int actualY = (int)(Grid.TOP*scaleHeight + row * HEIGHT*scaleHeight);
		g.setColor(color);
		g.fillRect(actualX, actualY, (int)(WIDTH*scaleWidth) + 1, (int)(HEIGHT*scaleHeight) + 1);
		// black border (if not empty)
		if (!color.equals(Grid.EMPTY)) {
			g.setColor(Color.BLACK);
			g.drawRect(actualX, actualY, (int)(WIDTH*scaleWidth), (int)(HEIGHT*scaleHeight));
		}
	}
}
