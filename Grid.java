...
public class Grid{
	...
	/**	 * Creates the grid	 */
	public Grid() {
		board = new Square[HEIGHT][WIDTH];		// put squares in the board
	...
	/**	 * Returns true if the location (row, col) on the grid is occupied* * @param row in the grid* @param col
 the column in the grid	 */
	public boolean isSet(int row, int col) {
		return !board[row][col].getColor().equals(EMPTY);
	}

	/**	 * Changes the color of the Square at the given location row
	...
	public void draw(Graphics g, double scaleWidth, double scaleHeight) {
		...
		g.fillRect((int)(scaleWidth*(LEFT - BORDER)), (int)(scaleHeight*TOP), (int)(scaleHeight*BORDER), (int)(scaleHeight*HEIGHT * Square.HEIGHT));
		g.fillRect((int)(scaleWidth*(LEFT + WIDTH * Square.WIDTH)), 
				(int)(scaleHeight*TOP), (int)(scaleHeight*BORDER), (int)(scaleHeight*HEIGHT
				* Square.HEIGHT));
		g.setColor(Color.BLUE);
		g.fillRect((int)(scaleWidth*(LEFT - BORDER)), (int)(scaleHeight*(TOP + HEIGHT * Square.HEIGHT)), (int)(scaleWidth*(WIDTH
				* Square.WIDTH + 2 * BORDER)), (int)scaleHeight*BORDER);
		...
		for (int r = 0; r < HEIGHT; r++) {
			for (int c = 0; c < WIDTH; c++) {
				if (board[r][c].getColor().equals(EMPTY)) {
					board[r][c].draw(g, scaleWidth, scaleHeight);
				}
			}
		}
		for (int r = 0; r < HEIGHT; r++) {
